...
def open_lc(fichier, fichier_results_salt):
    ...
    for i, t_name in enumerate(tableau['name']):
        if sn_name not in t_name:
            ...
            idx_name = i
    if idx_name is None:
        raise ValueError('No entry in %s found for the SN in %s' % (fichier_results_salt, fichier))
...
fichier = 'data_jla.nxt'

lcs_files = glob('/home/betoule/cosmo_jla1/data/lowz-2013-06-20_schlafly/*.list') + glob('/home/betoule/cosmo_jla1/data/salt2-SNLS3-SDSS3_v6/SDSS*.DAT') + glob('/home/betoule/cosmo_jla1/data/salt2-SNLS3-SDSS3_v6/lc*.list')
if os.path.exists(fichier):
    ...
    for i, lc in enumerate(entree):
        bands, mags, snr, instru = from_mb(lc)
        for j in range(len(bands)):
            data[k+j] = (lc['z'], lc['c'], lc['x0'], lc['x1'], np.nan, bands[j], i,
                         10**(-0.4*mags[j]), get_lambda_f(bands[j].split('_')[-1], instru), get_zp(bands[j].split('_')[-1], instru), snr[j])
        k += len(bands)
        if (i+1)%100 == 0:
...
class Model(object):
    ...
    def add_priors(self, Priors, el=1e-8):
        ...
        #mds = 0
        print mds[mds>self.intrinsic_dispersion**2]
        self.update_model('mB', np.identity(self.n_sn), (self.intrinsic_dispersion**2-mds)*np.identity(self.n_sn), w=1./(self.intrinsic_dispersion**2-mds)*np.identity(self.n_sn))
