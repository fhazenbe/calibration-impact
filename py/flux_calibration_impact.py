...
class Model(object):
    ...
    def construct_jacobian(self):
        ...
        if 'X0' not in fixed_pars:
            self.update_lines_2(np.arange(len(d)), p['X0'].indexof(d['#SN']), np.ones(len(d)))
...
if model_type == "wwa":
    par_list = [('color_law', len(color_law_params)), 'Omega_m', 'Omega_k', 'w', 'wa', 'H0', 'Omega_b_h2', 'beta'] + [('theta_salt', n+1),  ('X0', n_SN), ('c', n_SN), ('X1', n_SN), ('t0', n_SN)] + zp_list + dl_list
else:
    par_list = [('color_law', len(color_law_params)), 'Omega_m', 'Omega_lambda', 'w', 'H0', 'Omega_b_h2', 'beta'] + [('theta_salt', n+1),  ('X0', n_SN), ('c', n_SN), ('X1', n_SN), ('t0', n_SN)] + zp_list + dl_list
