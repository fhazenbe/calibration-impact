...
def create_survey(survey_type, mjd_min=59884., color=True, n=1, record=False, duration=None, max_SN=None, graph=False):
    log = [snsim.OpSimObsLog(create_log(survey_type, mjd_min+i*365.25, duration)) for i in range(n)]
    ...
    for i in range(n):
        ...
        if max_SN is not None:
            sne = np.random.choice(sne, max_SN)
        sne.sort(order='z')
        sne['X1'] = 0
        if color is False:
            sne['Color'] = 0
